...
import { buildIF06, printForm, mergeProjectLogos, getSignatureForName } from '../../utils/printEngine'
...
export default function IF06List() {
  ...
  const filtered = items.filter(d => {
    ...
    if (search) {
      ...
      return [d.if06_number, d.mockup_desc, d.activity_id, d.location].some(v => (v || '').toLowerCase().includes(q))
    }
    return true
  })
  ...
  const handlePrint = async (d) => {
    ...
    printForm(buildIF06({ ...mergeProjectLogos(d, activeProject), signatureImg }), 'IF06 — Sample Mockup Inspection')
  }
