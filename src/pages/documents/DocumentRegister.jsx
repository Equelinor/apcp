...
export default function DocumentRegister() {
  ...
  const filtered = docs.filter(d => {
    ...
    if (search) {
      ...
      return [d.doc_number, d.title, d.ref_number, d.activity_id, d.prepared_by].some(v => (v || '').toLowerCase().includes(q))
    }
    return true
  })
  ...
  const handlePrint = async (d) => {
    ...
    printForm(buildDSF({ ...mergeProjectLogos(d, activeProject), signatureImg, doc_number: printNumber, date: printDate, priorDate }), `Export for Transmittal — ${printNumber}`)
  }
            ...
            <tbody>
              {filtered.map(d => (
                <tr key={d.id}>
                  ...
                  <td style={{ fontSize: 12, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.title}</td>
