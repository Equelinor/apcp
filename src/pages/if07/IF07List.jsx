...
import { genDsfNumber, formatRevisedDsfNumber, SUBMITTAL_STATUSES, RESPONSE_CODES } from '../../config/docTypes'
...
// MAC's own resubmission-round status codes — same A/B/C/D/UR convention as MAC/SD
const REV_STATUS_CODES = ['', 'A', 'B', 'C', 'D', 'UR']

// ── Approval status — derived from response_code / status, not a stored field.
// Exported so Dashboard.jsx (or any future page) can reuse the exact same
// derivation, matching the RFI/Shop Drawing/IR/MAC registers' own pattern.
...
export function computeDsfApprovalStatus(d) {
  ...
  // dropdown, same rationale as MAC — a record with a real submitted_date but
  // status left at "Draft" should not still show as not-yet-submitted.
...
export function displayDsfNumber(d) {
  const latest = getLatestDsfRevision(d)
  return latest?.rev_no ? formatRevisedDsfNumber(d.if07_number, latest.rev_no) : (d.if07_number || '')
}
...
function exportDsfRegisterPDF(items, project) {
  ...
  const tableRows = withStatus.map((m, i) => {
    ...
    // this DSF's current state — same fallback-to-base pattern as MAC, built
    // in correctly from the start this time (MAC's own register export had
    // to be fixed after the fact for exactly this).
    ...
    return `<tr style="background:${bg}">
      ...
      <td style="border:0.4pt solid #ccc;padding:5pt 7pt;font-size:9.5pt">${m.doc_type || ''}</td>
...
<table>
  <thead>
    <tr>
      ...
      <th rowspan="2" style="border:0.5pt solid #aaa;padding:5pt;font-size:8.5pt;font-weight:700;background:#111827;color:#fff;width:10%">Type</th>
...
const BLANK = {
  if07_number: '',
  date: today(), doc_type: DOC_TYPES[0], title: '', ref_number: '',
  activity_id: '', activity_name: '', addressed_to: 'Consultant',
  prepared_by: '',
  // Left blank, not today() — an empty submitted_date is how the Register tells
  // Draft (not yet submitted) apart from Under Review, same rationale as MAC.
...
export default function IF07List() {
  ...
  async function loadData() {
    setLoading(true)
    const { data, error } = await supabase.from('if07').select('*').eq('project_code', activeProject.project_code).order('if07_number', { ascending: true })
  ...
  function suggestNextDsfNumber() {
    const nums = items
      ...
      .map(d => {
        const m = String(d.if07_number || '').match(/DSF-(\d+)/i)
  ...
  function openNew() { setEditItem(null); setForm({ ...BLANK, if07_number: suggestNextDsfNumber() }); setFormTab('details'); setShowForm(true) }
  ...
  async function save() {
    if (!form.if07_number?.trim()) { toast('DSF No. required', 'err'); return }
    if (!form.title) { toast('Title required', 'err'); return }
    // Empty string isn't valid for a date column — Postgres rejects it outright
    const payload = { ...form, if07_number: form.if07_number.trim(), response_date: form.response_date || null, submitted_date: form.submitted_date || null }
    if (editItem) {
      const { error } = await supabase.from('if07').update(payload).eq('id', editItem.id)
      ...
      toast('Document Submittal updated ✓', 'ok')
    } else {
      const item = { ...payload, project_code: activeProject.project_code }
      const { data, error } = await supabase.from('if07').insert(item).select().single()
      if (error) { toast('Save failed — ' + (error.code === '23505' ? 'that DSF No. is already used on this project' : error.message), 'err'); return }
      setItems(prev => [data, ...prev])
      toast(`Document Submittal created: ${item.if07_number}`, 'ok')
    }
    setShowForm(false)
  }
  ...
  const filtered = items.filter(d => {
    ...
    if (search) {
      ...
      return [d.if07_number, d.title, d.ref_number, d.activity_id].some(v => (v || '').toLowerCase().includes(q))
    }
    return true
  })

  // Once a DSF has left Draft, its content is locked for everyone except Admin —
  // same rule as MAC.
  ...
  const withDsfStatus = items.map(d => computeDsfApprovalStatus(d))
  const kpi = {
  ...
  const handlePrint = async (d) => {
    ...
    printForm(buildIF07({ ...mergeProjectLogos(d, activeProject), signatureImg, if07_number: printNumber, date: printDate, priorDate }), `Export for Transmittal — ${printNumber}`)
  }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">Document Submittal Form</div>
          <div className="page-subtitle">{activeProject.project_name} · IF07 (DSF) · {items.length} records</div>
        </div>
      ...
      <div className="filter-bar" style={{ marginBottom: 12 }}>
        <input placeholder="Search DSF number, title, ref…" value={search} onChange={e => setSearch(e.target.value)} />
      ...
      <div className="table-wrap">
        {loading ? <div className="table-empty">Loading…</div> : !filtered.length ? <div className="table-empty">No document submittals found.</div> : (
          <table>
            <thead>
              <tr>
                <th>DSF No.</th><th>Title</th><th>Type</th>
                <th>Ref No.</th>
            ...
            <tbody>
              {filtered.map(d => (
                <tr key={d.id}>
                  ...
                  <td style={{ fontSize: 12, maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.title}</td>
                  <td style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{d.doc_type}</td>
                  <td style={{ fontSize: 12 }}>{d.ref_number || '—'}</td>
        ...
        <div>
          <div className="form-grid form-grid-2" style={{ gap: 14, marginBottom: 14 }}>
            <div className="form-group">
              <label className="form-label required">DSF No. <span style={{ fontWeight: 400, color: 'var(--text-muted)', fontSize: 11 }}>(auto-suggested, editable — e.g. append a/b/c for split submissions)</span></label>
              <input className="form-input" value={form.if07_number} disabled={isLocked} onChange={e => set('if07_number', e.target.value)} style={{ fontFamily: 'var(--font-mono)', fontWeight: 700 }} />
            </div>
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={form.doc_type} disabled={isLocked} onChange={e => set('doc_type', e.target.value)}>
                    ...
                    {form.submission_history.map((r, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: '6px 10px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>
                          {formatRevisedDsfNumber(form.if07_number, r.rev_no)}
                        </td>
                        <td style={{ padding: '6px 10px' }}>
